...
interface LiquidGlassShaderProps {
  ...
  onReady?: () => void;
  alpha?: boolean; // 투명 배경 사용 여부
}

export default function LiquidGlassShader({ backgroundMedia, uniforms, className, isTransitioning = false, onReady, alpha = false }: LiquidGlassShaderProps) {
  ...
  const getFragmentShaderSource = useCallback((hasDerivatives: boolean, useAlpha: boolean) => `
    ...
    void main() {
      ...
      vec3 finalColor = mix(shadowedBackground, blurredTex, vec3(boxMask));
      float alpha = ${useAlpha ? 'boxMask > 0.01 ? 1.0 : 0.0' : '1.0'};
  ...
  const fragmentShaderSource = useMemo(() => getFragmentShaderSource(hasDerivatives, alpha), [hasDerivatives, alpha, getFragmentShaderSource]);
